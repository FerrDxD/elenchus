...
<div class="min-h-screen bg-[var(--color-background)] p-8 relative flex flex-col items-center justify-center overflow-auto">
  <!-- Decorative grid -->
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
            {#each levels.filter(l => l.group === group) as level}
              ...
              >
                ...
                <span class="text-xs text-[var(--color-muted-foreground)] group-hover:text-[var(--color-primary)] transition-colors">SYS.{level.order}</span>
